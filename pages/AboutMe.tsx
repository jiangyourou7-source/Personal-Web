    ...
    <div className="page-shell py-20 lg:py-24">
      ...
      <section className="grid gap-10 py-14 lg:grid-cols-[0.72fr_1.28fr]">
        <div className="paper-card rounded-[28px] bg-white p-5">
          <img src={PROFILE.avatarSecondary} alt={PROFILE.name} className="w-full rounded-[20px] object-cover" />
        </div>

        <div className="grid gap-8">
          <div className="paper-card rounded-[28px] bg-white p-8">
            <h2 className="font-[var(--font-family-display)] text-3xl font-bold text-[var(--color-text)]">我的工作方式</h2>
            <div className="mt-6 grid gap-5">
              {[
                ...
                <div key={item} className="rounded-2xl bg-[var(--color-surface-soft)] px-5 py-4 text-sm leading-7 text-[var(--color-text-muted)]">
                  {item}
                </div>
              ))}
            </div>
          ...
          <div className="paper-card rounded-[28px] bg-white p-8">
            <h2 className="font-[var(--font-family-display)] text-3xl font-bold text-[var(--color-text)]">别人为什么愿意找我合作</h2>
            <div className="mt-6 space-y-5">
              {[
                ...
                <blockquote key={item} className="border-l-2 border-[var(--color-primary)] pl-5 text-base leading-8 text-[var(--color-text-muted)]">
                  {item}
                </blockquote>
              ))}
            </div>
